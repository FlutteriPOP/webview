...
class AndroidNavigationDelegate extends PlatformNavigationDelegate {
  ...
  void _handleNavigation(
    ...
    // The client can only intercept navigations for the main frame.
    if (!isForMainFrame ||
    ...
    final FutureOr<NavigationDecision> result = onNavigationRequest(
      NavigationRequest(
        url: url,
    ...
    if (result is NavigationDecision) {
      if (result == NavigationDecision.navigate) {
        onLoadRequest(
          LoadRequestParams(
            uri: Uri.parse(url),
            headers: headers,
      ...
      result.then((NavigationDecision decision) {
